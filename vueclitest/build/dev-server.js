...
if (!process.env.NODE_ENV) { // 如果 Node 的环境无法判断当前是 dev / product 环境
  process.env.NODE_ENV = JSON.parse(config.dev.env.NODE_ENV)  // 使用 config.dev.env.NODE_ENV 作为当前的环境
}
...
var devMiddleware = require('webpack-dev-middleware')(compiler, {  // 启动 webpack-dev-middleware，将 编译后的文件暂存到内存中
  publicPath: webpackConfig.output.publicPath,
  quiet: true
})

var hotMiddleware = require('webpack-hot-middleware')(compiler, {  // 启动 webpack-hot-middleware，也就是我们常说的 Hot-reload
  log: () => {}
})
...
app.use(hotMiddleware)  // 将 Hot-reload 挂在到 express 服务上

// serve pure static assets
var staticPath = path.posix.join(config.dev.assetsPublicPath, config.dev.assetsSubDirectory)  // 拼接 static 文件夹的静态资源路径
app.use(staticPath, express.static('./static'))  // 为静态资源提供响应服务
